    ...
    <div className="space-y-5 p-4 sm:p-6 text-slate-100">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
        ...
        <div className="bg-slate-900 border border-amber-500/50 p-3 rounded-lg bg-gradient-to-br from-amber-950/30 to-slate-900">
          ...
          <span className="text-[10px] text-amber-400/80">Radar alerts active</span>
        </div>
      ...
      {/* "The Safety Gap" Innovation Panel */}
      <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="self-start md:self-auto bg-slate-800 hover:bg-slate-700 text-slate-200 px-3 py-1.5 rounded font-medium text-xs flex items-center gap-1.5 transition-all border border-slate-700"
        >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-900 p-3 rounded-lg border border-slate-800">
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-md pl-9 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-rose-500"
          />
        </div>
      ...
      {/* Clinical Priority Board Grid */}
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs font-mono text-slate-400 px-1">
          <span>Clinical Priority Board ({sortedPatients.length} active records)</span>
          <span>Sorted by Risk Score & Severity</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {sortedPatients.map((patient) => {
            ...
            return (
              <div
                key={patient.id}
                className={`bg-slate-900 rounded-xl border transition-all duration-200 p-4 space-y-3 relative group hover:border-slate-700 shadow-lg ${
                  isCritical 
                    ? 'border-rose-500/80 bg-gradient-to-b from-rose-950/30 to-slate-900' 
                    : isHigh 
              ...
              >
                ...
                {/* Risk Score, Confidence & Uncertainty Metrics */}
